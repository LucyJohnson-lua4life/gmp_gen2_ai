...
import {IDailyRoutineComponent} from '../../scripts/aiEntities/components/iDailyRoutineComponent';
import {DailyRoutineSystem, DrTimeTriggerInfo} from '../../scripts/aiFunctions/dailyRoutineSystem';

test('hour & minute overlap trigger period first time ... SHOULD RETURN TRUE', () => {
    let botId = 1;
    let entityManager:EntityManager = new EntityManager();
    let routineInfoComponent:IDailyRoutineComponent = {entityId: botId, startHour: 0, startMinute: 0, endHour: 0, endMinute: 0, lastHour: 0, lastMinute: 0}
...
test('hour & minute overlap trigger period but not for the first time ... SHOULD RETURN false', () => {
    let botId = 1;
    let entityManager:EntityManager = new EntityManager();
    let routineInfoComponent:IDailyRoutineComponent = {entityId: botId, startHour: 12, startMinute: 30, endHour: 15, endMinute: 0, lastHour: 0, lastMinute: 0}
    entityManager.setDailyRoutineComponent(botId, routineInfoComponent)
...
test('hour & minute overlap trigger period NOT first time, while start hour > end hour of trigger period AND current time > start trigger period ... SHOULD RETURN false', () => {
    let botId = 1;
    let entityManager:EntityManager = new EntityManager();
    let routineInfoComponent:IDailyRoutineComponent = {entityId: botId, startHour: 23, startMinute: 30, endHour: 2, endMinute: 0, lastHour: 0, lastMinute: 0}
    entityManager.setDailyRoutineComponent(botId, routineInfoComponent)
    let drTimeInfo:DrTimeTriggerInfo = {currentHour:23, currentMinute:40, startHour:23, startMinute:30, endHour:2, endMinute:0};
    let handler:DailyRoutineSystem = new DailyRoutineSystem(entityManager);
    expect(handler.newDrTimePeriodEntered(botId, drTimeInfo)).toBe(false);
...
test('trigger period starts and ends from 0-24. 24h are over ... SHOULD RETURN true', () => {
    let botId = 1;
    let entityManager:EntityManager = new EntityManager();
    let routineInfoComponent:IDailyRoutineComponent = {entityId: botId, startHour: 0, startMinute: 0, endHour: 24, endMinute: 0, lastHour: 23, lastMinute: 0}
    entityManager.setDailyRoutineComponent(botId, routineInfoComponent)
...
test('trigger period starts and end at 0 o clock. 24h not over ... SHOULD RETURN false', () => {
    let botId = 1;
    let entityManager:EntityManager = new EntityManager();
    let routineInfoComponent:IDailyRoutineComponent = {entityId: botId, startHour: 0, startMinute: 0, endHour: 24, endMinute: 0, lastHour: 21, lastMinute: 0}
    entityManager.setDailyRoutineComponent(botId, routineInfoComponent)
